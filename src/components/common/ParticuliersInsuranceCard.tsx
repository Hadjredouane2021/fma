...
import type { InsuranceCard } from "@/lib/particuliers-site-public";
import { resolveInsuranceCardLink } from "@/lib/particuliers-site-public";
import { isLaFmaIconImage, isLaFmaUploadIcon, normalizeLaFmaIcon } from "@/lib/la-fma-icon";
import type { Locale } from "@/types";

type ParticuliersInsuranceCardProps = {
  card: InsuranceCard;
  locale: Locale;
  localePath: string;
};
...
export function ParticuliersInsuranceCard({ card, locale, localePath }: ParticuliersInsuranceCardProps) {
  const title = card.title[locale] || card.title.fr;
  const desc = card.description[locale] || card.description.fr;
  const feats = card.features.map((f) => f[locale] || f.fr).filter(Boolean);
  const detailHtml = (card.detailContent?.[locale] || card.detailContent?.fr || "").trim();
  const hasModal = detailHtml.length > 0;
  const learnMoreHref = resolveInsuranceCardLink(card.link, localePath);
  const isExternalLink = /^https?:\/\//i.test(learnMoreHref);
  const buttonLabel = learnMoreLabel(locale);
  const iconValue = normalizeLaFmaIcon(card.icon);
  ...
  return (
    <div className="glass-liquid rounded-2xl p-7 card-hover">
      <div className="relative z-10 flex items-start gap-5">
        <ParticuliersCardIcon icon={card.icon} color={card.color} />
