...
type Title = { fr: string; en: string; ar: string };
type Item = { url: string; link: string };

export default function ConseilFmaGalleryForm({ initial, initialTitle }: { initial: Item[]; initialTitle: Title }) {
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch("/api/admin/site/conseil-fma", {
        method: "PUT",
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 mb-8">
      <h2 className="text-sm font-bold text-primary mb-4 flex items-center gap-2">
        ...
        Carrousel — Le Conseil FMA (page d&apos;accueil)
      </h2>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        <div>
          <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Titre (FR)</label>
          <input type="text" value={title.fr} onChange={(e) => setTitle((t) => ({ ...t, fr: e.target.value }))} className={inputBase} placeholder="Le Conseil FMA" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Titre (EN)</label>
          <input type="text" value={title.en} onChange={(e) => setTitle((t) => ({ ...t, en: e.target.value }))} className={inputBase} placeholder="The FMA Council" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Titre (AR)</label>
          <input type="text" dir="rtl" value={title.ar} onChange={(e) => setTitle((t) => ({ ...t, ar: e.target.value }))} className={inputBase} placeholder="مجلس الاتحاد" />
        </div>
      </div>
