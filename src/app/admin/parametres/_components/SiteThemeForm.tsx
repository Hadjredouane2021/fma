...
const COLOR_FIELDS: { key: keyof SiteThemeColors; label: string }[] = [
  { key: "brand", label: "Couleur principale (bordeaux)" },
  { key: "blue", label: "Accent (bleu) — aussi utilisé pour les titres de section" },
  { key: "mauve", label: "Mauve" },
  { key: "gold", label: "Or / Taupe" },
  { key: "graphite", label: "Graphite (anthracite)" },
  { key: "pale", label: "Bleu pâle" },
];

export default function SiteThemeForm({ initial }: { initial: SiteThemeSettings }) {
  ...
  const setColor = (mode: "light" | "dark", key: keyof SiteThemeColors, value: string) => {
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch("/api/admin/theme/site", {
        method: "PUT",
  ...
  return (
    <div className="glass-liquid max-w-2xl rounded-2xl p-6 card-hover">
      ...
      <p className="relative z-10 mb-6 text-xs text-[var(--text-3)]">
        Palette utilisée sur le site visiteurs, en mode clair et sombre.
      </p>
