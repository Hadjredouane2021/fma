...
export default function VocabulaireHeroImageForm({ initial }: { initial: string }) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append("file", file);
      fd.append("folder", "vocabulaire-hero");
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch("/api/admin/site/vocabulaire-hero", {
        method: "PUT",
    ...
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 mb-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          ...
          placeholder="/uploads/vocabulaire-hero/…"
          disabled={uploading}
        />
        <label
          className={cn(buttonUploadLabel, "inline-flex items-center gap-2", uploading && "opacity-60 pointer-events-none")}
        >
          <input
            type="file"
      ...
      <div className="flex items-center gap-4">
        ...
        {success ? <span className="text-sm text-emerald-600 dark:text-emerald-400 font-medium">Enregistré ✓</span> : null}
