...
export default function ConventionsHeroImageForm({
  initial = EMPTY_CONVENTIONS_HERO_IMAGE_URLS,
}: {
  initial?: ConventionsHeroImageUrls;
}) {
  const router = useRouter();
  const [images, setImages] = useState<ConventionsHeroImageUrls>(initial);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append("file", file);
      fd.append("folder", "conventions-hero");
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch("/api/admin/site/conventions-hero", {
        method: "PUT",
    ...
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 mb-8">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-1">
        <h2 className="text-sm font-bold text-primary flex items-center gap-2">
          ...
          Image hero — page Conventions professionnelles
        </h2>
      ...
      <p className="text-xs text-[var(--text-3)] mb-4">
        Affichée en haut de la page Conventions professionnelles pour l&apos;onglet{" "}
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          ...
          placeholder={`/uploads/conventions-hero/… (${tab.toUpperCase()})`}
          disabled={uploading}
        />
