...
export default function ParticuliersHeroImageForm({
  initial = EMPTY_PARTICULIERS_HERO_IMAGE_URLS,
}: {
  initial?: ParticuliersHeroImageUrls;
}) {
  const router = useRouter();
  const [images, setImages] = useState<ParticuliersHeroImageUrls>(initial);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append("file", file);
      fd.append("folder", "particuliers-hero");
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch("/api/admin/site/particuliers-hero", {
        method: "PUT",
    ...
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 mb-8">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-1">
        <h2 className="text-sm font-bold text-primary flex items-center gap-2">
          ...
          Image hero — page Espace Particuliers
        </h2>
      ...
      <p className="text-xs text-[var(--text-3)] mb-4">
        Affichée en haut de la page Espace Particuliers pour l&apos;onglet{" "}
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          ...
          placeholder={`/uploads/particuliers-hero/… (${tab.toUpperCase()})`}
          disabled={uploading}
        />
