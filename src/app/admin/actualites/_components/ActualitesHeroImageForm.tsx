...
export default function ActualitesHeroImageForm({
  initial = EMPTY_ACTUALITES_HERO_IMAGE_URLS,
}: {
  initial?: ActualitesHeroImageUrls;
}) {
  const router = useRouter();
  const [images, setImages] = useState<ActualitesHeroImageUrls>(initial);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append("file", file);
      fd.append("folder", "actualites-hero");
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch("/api/admin/site/actualites-hero", {
        method: "PUT",
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 mb-8">
      ...
      <p className="text-xs text-[var(--text-3)] mb-4">
        Affichée en haut de la page Actualités pour l&apos;onglet <strong>{currentTab.label}</strong>.
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          ...
          placeholder={`/uploads/actualites-hero/… (${tab.toUpperCase()})`}
          disabled={uploading}
        />
