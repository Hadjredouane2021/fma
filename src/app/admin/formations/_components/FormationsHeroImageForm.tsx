...
export default function FormationsHeroImageForm({
  initial = EMPTY_FORMATIONS_HERO_IMAGE_URLS,
}: {
  initial?: FormationsHeroImageUrls;
}) {
  const router = useRouter();
  const [images, setImages] = useState<FormationsHeroImageUrls>(initial);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append("file", file);
      fd.append("folder", "formations-hero");
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch("/api/admin/site/formations-hero", {
        method: "PUT",
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 mb-8">
      ...
      <p className="text-xs text-[var(--text-3)] mb-4">
        Affichée en haut de la page Formations pour l&apos;onglet{" "}
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          ...
          placeholder={`/uploads/formations-hero/… (${tab.toUpperCase()})`}
          disabled={uploading}
        />
