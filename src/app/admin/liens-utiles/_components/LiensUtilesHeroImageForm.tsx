...
import {
  EMPTY_LIENS_UTILES_HERO_IMAGE_URLS,
  type LiensUtilesHeroImageUrls,
...
export default function LiensUtilesHeroImageForm({
  initial = EMPTY_LIENS_UTILES_HERO_IMAGE_URLS,
}: {
  initial?: LiensUtilesHeroImageUrls;
}) {
  const router = useRouter();
  const [images, setImages] = useState<LiensUtilesHeroImageUrls>(initial);
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 mb-8">
      ...
      <p className="text-xs text-[var(--text-3)] mb-4">
        Affichée en haut de la page Liens utiles pour l&apos;onglet{" "}
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          ...
          placeholder={`/uploads/liens-utiles-hero/… (${tab.toUpperCase()})`}
          disabled={uploading}
        />
